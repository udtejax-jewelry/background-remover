...
# ---- Optional: load GEMINI_API_KEY from .env
try:
...
ALLOWED_DOMAINS = {e.split("@", 1)[1] for e in ALLOWED_EMAILS}  # strict domain set
SHARED_PASSWORD = "udny123"
...
def is_allowed_email(email: str) -> bool:
    ...
    # strict domain: must be one of the allowed domains
    if "@" not in email_n:
        return False
    local, domain = email_n.split("@", 1)
    if domain not in ALLOWED_DOMAINS:
        return False
    # exact allowed emails list (case-insensitive for local part)
...
# =========================
# Gemini client (for Step-1 preview)
# =========================
